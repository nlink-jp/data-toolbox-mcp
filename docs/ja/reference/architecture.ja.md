...
本書は `data-toolbox-mcp` の全体設計を、Phase 0 で確定した ADR-0001〜0005 + v0.2.0 で追加した ADR-0006 / ADR-0007 + v0.3.0 で追加した ADR-0008 / ADR-0009 + v0.4.0 で追加した ADR-0010 を前提に記述する。実装後の細部（ライブラリ選定・関数名・JSON Schema の確定形）は対象外。
...
5. MCP server: 行数が LIMIT に達していたら、追加で `SELECT COUNT(*) FROM (user_sql) sub` を実行して総行数を取得 (v0.4.0 / ADR-0010)
6. 戻り値に {rows, row_count, limit_applied, limit_reached, truncated, total} を返却。truncate されていなければ total = row_count、truncate されている場合は内部 COUNT() の結果。COUNT() が timeout したときは total: null + total_unavailable_reason
7. テーブル不在エラー (`CatalogException: Table ... does not exist`) を検出したら、SHOW TABLES + Manager.List() で hint を作って構造化エラーの details に missing_table / available_tables_in_this_workspace / other_workspaces を添える (v0.4.0)
```
...
3. dry_run = true (v0.4.0) のとき:
   ...
   - host_paths と disk_usage_bytes (ディレクトリツリーの walk 合計) を計算
   - 何も削除せず {would_delete, container_id, container_state, host_paths, disk_usage_bytes} を返却
4. dry_run = false (既定、既存挙動):
...
dry_run = false は不可逆。MCP クライアント側のユーザー承認に加え、dry_run = true で「これを消すよ」を LLM がユーザーに見せられる二重防御。
...
```
1. MCP server: workspace_id 検証 (Ensure はせず disk のみ確認)
2. 各 path について:
   - "/work/<sub>" or "<sub>" を <host_work_dir>/<sub> に解決
   - filepath.Clean + <host_work_dir> prefix check で path traversal 二重防御
   - os.Stat で存在 + サイズ取得
3. 拡張子から種別判定:
   - .png/.jpg/.svg/... → image content block (base64 + mimeType)
   - .csv/.json/.md/... → text content block (size 上限超過時は head + tail 省略)
   - その他 → metadata-only text block (host path + size + sha256)
4. 単体 max_single_size_bytes / 合計 max_total_size_bytes (config) を超えるものは metadata-only に降格
5. MCP content 配列を組み立て、mcpserver.RawResult として handleToolsCall に返す
6. handleToolsCall は RawResult なら content をそのまま使う (通常ツールは JSON.Marshal して single text)
```

Ensure 不要 (Podman exec しない、ホストファイル読み出しのみ)。LLM が `execute_code` で生成した plot 等を「Claude Desktop のフォルダ接続なしでもインライン表示」する用途。

### 3.8 load_from_work(workspace_id, file_path, table_name) — v0.3.0 (ADR-0009)

```
1. MCP server: workspace_id 検証
2. file_path が "/work/" で始まることを検証 (それ以外は invalid_arguments)
3. /work プレフィックスを剥がして <host_work_dir>/<sub> に解決
4. filepath.Clean + <host_work_dir> prefix check で path traversal 二重防御
5. workspace を Ensure
6. 拡張子から reader 選択 (load_data と同じテーブル: csv/json/parquet)
7. Python script を execScript で実行:
   CREATE OR REPLACE TABLE "<table>" AS SELECT * FROM read_xxx_auto('<container_path>')
   SELECT COUNT(*) + DESCRIBE で行数とスキーマを取得
...
```
1. MCP server: workspace_id 検証 + Ensure
2. podman exec で Python に SHOW TABLES + 各テーブル DESCRIBE を依頼
3. Python: tables = con.execute("SHOW TABLES").fetchall()
            schemas = {t: con.execute(f'DESCRIBE "{t}"').fetchall() for t in tables}
   を JSON 化して stdout
4. {workspace_id, host_work_dir, container_state, tables: [{name, columns: [{name, type}]}]} を返却
```

`list_workspaces` (workspace 一覧) と `describe_workspace` (1 workspace の中身) の対称形。row_count は含めない (v0.4.0 では、SELECT COUNT 全テーブル分は重い、需要が出たら別 ADR)。
