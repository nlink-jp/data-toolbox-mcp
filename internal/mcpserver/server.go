...
//
// Tool implementations live in internal/tools (Phase 1 Track D); the server
// only routes them via the RegisterTool API.
package mcpserver
...
func (s *Server) Serve(ctx context.Context) error {
	for {
		...
		if err := s.handle(ctx, line); err != nil {
			// Per memory feedback_mcp_proxy_always_responds, handle() always
			// either replies or (for notifications) intentionally stays silent.
			// An error here means the write itself failed; log and continue.
