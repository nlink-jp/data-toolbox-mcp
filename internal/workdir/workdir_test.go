...
// TestSensitiveNamesTheCredentialLocations covers the read blacklist: a file
// to load may be an absolute path anywhere readable, so the floor has to hold
// here. The list is the runtimes' (~/.kube, ~/.netrc are new with ADR-0004).
func TestSensitiveNamesTheCredentialLocations(t *testing.T) {
	...
	// refuse these).
	for _, p := range []string{filepath.Join(home, "Downloads", "sales.csv"), "/private/tmp/x.parquet",
