...
const workDirProp = `"work_dir": {"type":"string","description":"Absolute path to a directory you can read back \u2014 your session or working directory. The workspace is <work_dir>/<workspace_id>/ and everything this server writes for you lands under it, including files execute_code creates. It must already exist, and nothing here expands ~ or resolves a relative path."}`
...
func Register(srv *mcpserver.Server, mgr *workspace.Manager, cfg *config.Config) {
	srv.RegisterTool(mcpserver.Tool{
		Name:        "load_data",
		Description: "Load a host file into the workspace's DuckDB as a table. file_path may be any file you can read, except credential locations such as ~/.ssh. For files generated by execute_code that already live in /work, use load_from_work instead.",
	...
	srv.RegisterTool(mcpserver.Tool{
		Name:        "query_data",
		Description: "Run SQL against the workspace's DuckDB and return rows as JSON. If the SQL has no LIMIT, an outer LIMIT (default 20000) is auto-applied; the result carries truncated=true and total=N when this trips. Missing-table errors include a hint with the workspace's actual tables and other workspaces' names.",
		InputSchema: json.RawMessage(`{
			"type": "object",
			"properties": {
				` + workDirProp + `,
			...
			"required": ["work_dir","workspace_id","sql"]
		}`),
	}, func(ctx context.Context, args json.RawMessage) (any, error) {
	...
	srv.RegisterTool(mcpserver.Tool{
		Name:        "execute_code",
		Description: "Run Python code inside the workspace's container. Bundled libraries: duckdb, pandas, polars, pyarrow, matplotlib, Pillow. DuckDB file is at /work/analysis.duckdb. Files written to /work appear on the host at the returned host_work_dir, which is inside the work_dir you named; return them to the user with attach_files (do NOT base64-encode into stdout).",
		InputSchema: json.RawMessage(`{
			"type": "object",
			"properties": {
				` + workDirProp + `,
			...
			"required": ["work_dir","workspace_id","language","code"]
		}`),
	}, func(ctx context.Context, args json.RawMessage) (any, error) {
	...
	srv.RegisterTool(mcpserver.Tool{
		...
		Description: "List every workspace that has on-disk state under your work_dir (id, last_used, container_state). Use this to discover prior workspaces across chat sessions. Only directories that really are workspaces are listed — anything else under work_dir is not one.",
		InputSchema: json.RawMessage(`{
			"type": "object",
			"properties": {
				` + workDirProp + `
	...
	srv.RegisterTool(mcpserver.Tool{
		...
		Description: "Stop the container (if any) and wipe a workspace's on-disk state completely. IRREVERSIBLE when dry_run is false (the default). Set dry_run: true to see what would be removed (container_id, host_paths, disk_usage_bytes) without acting.",
		InputSchema: json.RawMessage(`{
			"type": "object",
			"properties": {
				` + workDirProp + `,
				"workspace_id": {"type":"string","description":"Workspace key to delete. Must match ^[a-zA-Z0-9_-]{1,64}$."},
				"dry_run":      {"type":"boolean","description":"If true, report what would be deleted instead of deleting. Default false."}
			},
	...
	srv.RegisterTool(mcpserver.Tool{
		...
		InputSchema: json.RawMessage(`{
			...
			"required": ["work_dir","workspace_id","paths"],
			"additionalProperties": false
		}`),
	}, func(ctx context.Context, args json.RawMessage) (any, error) {
	...
	// --- v0.4.0 tools (ADR-0010) ---

	srv.RegisterTool(mcpserver.Tool{
		Name:        "describe_workspace",
		Description: "List every user table in the workspace's DuckDB with its column schema. Pair with list_workspaces: list_workspaces enumerates workspaces, describe_workspace drills into one. Row counts are not returned (separate query if needed).",
		InputSchema: json.RawMessage(`{
			"type": "object",
			"properties": {
				` + workDirProp + `,
		...
		return DescribeWorkspace(ctx, mgr, cfg, args)
	})
}
