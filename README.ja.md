...
- `/work` は `execute_code` が走らせるコードから書き込めるので、そこにある symlink はサンドボックスからの入力として扱う。サーバーは workspace のファイルに work ディレクトリの `os.Root` 経由でしか触れない: `attach_files` はリンク経由で外へ出るパスを拒否し、サーバー自身の書き込み（`_upload/`・`_code/`）もリンクで行き先を変えられない。`/work` の内側に留まるリンクは従来どおり使える。
